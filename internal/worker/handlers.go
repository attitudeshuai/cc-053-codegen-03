...
func (p *Processor) ProcessRecording(ctx context.Context, t *asynq.Task) error {
	...
	if sampleRate != 16000 {
		p.recordingRepo.Reject(payload.RecordingID, "system:qc", "sample rate must be 16kHz")
		return fmt.Errorf("invalid sample rate: %d", sampleRate)
	}

	// Validate peak level
	if peakDB > -1.0 {
		p.recordingRepo.Reject(payload.RecordingID, "system:qc", fmt.Sprintf("peak level too high: %.2f dB", peakDB))
